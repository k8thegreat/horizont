...
if ($bAllowAccess)
{
	...
	while ($arProperty = $rsIBLockPropertyList->GetNext())
	{
		...
		{
			$rsIBlockElements = CIBlockElement::GetList(array("SORT" => "ASC"), array("IBLOCK_ID" => $arProperty["LINK_IBLOCK_ID"], "ACTIVE" => "Y"), false, false, array("ID", "NAME"));
			while ($ob = $rsIBlockElements->GetNext())
			{
				$arProperty["ENUM"][$ob["ID"]] = $ob;
			}
	...
	if ($arParams["ELEMENT_ASSOC"] == "PROPERTY_ID" && strlen($arParams["ELEMENT_ASSOC_PROPERTY"]) && !$USER->IsAdmin())
	{
		if ($arParams["ELEMENT_ASSOC_PROPERTY_VALUE"])
			$arFilter["PROPERTY_".$arParams["ELEMENT_ASSOC_PROPERTY"]] = $arParams["ELEMENT_ASSOC_PROPERTY_VALUE"];
		else
			$arFilter["ID"] = -1;
	}
	elseif ($USER->GetID())
	...
	if (check_bitrix_sessid() && (!empty($_REQUEST["iblock_submit"]) || !empty($_REQUEST["iblock_apply"])))
	{
		...
		if (empty($arResult["ERRORS"]))
		{
			if ($arParams["ELEMENT_ASSOC"] == "PROPERTY_ID")
				if($arParams["ELEMENT_ASSOC_PROPERTY_VALUE"])
				$arUpdatePropertyValues[$arParams["ELEMENT_ASSOC_PROPERTY"]] = $arParams["ELEMENT_ASSOC_PROPERTY_VALUE"];
				else 
			...
			if ($arParams["ID"] > 0)
			{
				...
				if($ar_res = $res->GetNext()){
				  	$activeFlagVal = $ar_res["ACTIVE"];
				}
				$property_enums = CIBlockPropertyEnum::GetList(Array(), Array("IBLOCK_ID"=>$arParams["IBLOCK_ID"], "CODE"=>"MODERATE"));
				if($enum_fields = $property_enums->GetNext())
				{
				  	$moderateFlagVal = $enum_fields["ID"];
				}
				
				$arModerateFileds = serialize($arUpdateValues);
				if($activeFlagVal=="N"){
					...
					if(!$oElement->SetPropertyValuesEx($arParams["ID"], false, 
						array(
							"ACTIVE" => $activeFlagVal, 
							"MODERATE" => $moderateFlagVal,
							"MODERATE_FIELDS" => array('VALUE'=>array('TYPE'=>'TEXT', 'TEXT'=>$arModerateFileds))
						)
						) && $oElement->Update($arParams["ID"], array("ACTIVE"=>$activeFlagVal))
					){
						$arResult["ERRORS"][] = $oElement->LAST_ERROR;
					}
				}
			...
			else
			{
				...
				$rsProp = CIBlockProperty::GetList(Array("sort"=>"asc", "name"=>"asc"), Array("ACTIVE"=>"Y", "IBLOCK_ID"=>$arParams["IBLOCK_ID"]));
					while ($arr=$rsProp->Fetch()){//print_r($arr);
						if($arr["PROPERTY_TYPE"]=='L'){
							$db_enum_list = CIBlockProperty::GetPropertyEnum($arr["CODE"], Array(), Array("IBLOCK_ID"=>$arParams["IBLOCK_ID"]));
							while($ar_enum_list = $db_enum_list->GetNext()){
							  $arPropertyListValues[$arr["ID"]][$ar_enum_list["ID"]]=$ar_enum_list["VALUE"];
							}	
						}
						elseif($arr["PROPERTY_TYPE"]=='E'){
							$arFilter=array("IBLOCK_ID"=>$arr["LINK_IBLOCK_ID"],"ACTIVE"=>"Y");
							$arSelect=array("NAME", "ID");
							$res = CIBlockElement::GetList(Array(), $arFilter, false, false, $arSelect);
							while($ar_fields = $res->GetNext())
							{
							  $arPropertyListValues[$arr["ID"]][$ar_fields["ID"]]=$ar_fields["NAME"];
							}
						}
						$arProperty[$arr["ID"]] = $arr["CODE"];
					}

				$arFullPropertyCodes = $arProperty;
				foreach ($arUpdateValues["PROPERTY_VALUES"] as $id=>$prop){
						if(is_array($arPropertyListValues[$id])){
							if(is_array($prop)){
								
								foreach ($prop as $prop_val)
								$arrLetterFields[$arProperty[$id]][]=$arPropertyListValues[$id][$prop_val];
								$arrLetterFields[$arProperty[$id]]=implode(', ',$arrLetterFields[$arProperty[$id]]);
							}else{
								$arrLetterFields[$arProperty[$id]]=$arPropertyListValues[$id][$prop];
							}				
						}
						else
							$arrLetterFields[$arProperty[$id]]=$prop;
				...
				if (!$arParams["ID"] = $oElement->Add($arUpdateValues, $bWorkflowIncluded, true, $arParams["RESIZE_IMAGES"]))
				{
                        ...
                        $arEventFields = $arrLetterFields;
                        $arEventFields["ID"]=$arParams["ID"];
                        $arEventFields["IBLOCK_ID"]=$arParams["IBLOCK_ID"];
                        $arEventFields["IBLOCK_TYPE"]=$arParams["IBLOCK_TYPE"];
                        $arEventFields["NAME"]=$arUpdateValues["NAME"];
                        $arEventFields["PREVIEW_TEXT"]=$arUpdateValues["PREVIEW_TEXT"];
                        $site_url = $_SERVER["SERVER_NAME"];
                        foreach ($arResult["PROPERTY_LIST_FULL"] as $propertyID => $value) {
                        	if($value["PROPERTY_TYPE"] == "F"){
                        		$file_arr = array();
                        		$db_props = CIBlockElement::GetProperty($arParams["IBLOCK_ID"], $arParams["ID"], array("sort" => "asc"), Array("ID"=>$propertyID));
								while($ar_props = $db_props->Fetch()){
									$file = CFile::GetFileArray($ar_props["VALUE"]);
								    $file_arr[] = '<a target="_blank" href="http://'.$site_url.$file["SRC"].'">'.$file["ORIGINAL_NAME"].'</a>';
								}

								$arEventFields[$arFullPropertyCodes[$propertyID]] = implode(", ", $file_arr);
								
                        	}
                        	
                        }
                        if (CEvent::Send($arParams["EVENT_NAME"], SITE_ID, $arEventFields)){
